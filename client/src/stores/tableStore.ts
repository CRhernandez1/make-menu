...
async function parseError(response: Response, fallback: string): Promise<string> {
  ...
  // Mensajes descriptivos según el código HTTP
...
export const useTableStore = defineStore('tables', () => {
  ...
  const fetchTables = async (cif: string) => {
    ...
    error.value = null
    try {
      const response = await fetch(`${API_URL}/establishments/${cif}/tables/`)
      if (!response.ok) {
  ...
  const createTable = async (cif: string, data: TableCreate): Promise<ActionResult> => {
    try {
      const response = await fetch(`${API_URL}/establishments/${cif}/tables/add/`, {
        method: 'POST',
  ...
  const updateTable = async (cif: string, tableNum: number, data: TableUpdate): Promise<ActionResult> => {
    try {
      const response = await fetch(`${API_URL}/establishments/${cif}/tables/${tableNum}/edit/`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const toggleTable = async (cif: string, tableNum: number): Promise<ActionResult> => {
    try {
      const response = await fetch(`${API_URL}/establishments/${cif}/tables/${tableNum}/change/`)
      if (!response.ok) {
  ...
  const deleteTable = async (cif: string, tableNum: number): Promise<ActionResult> => {
    try {
      const response = await fetch(`${API_URL}/establishments/${cif}/tables/${tableNum}/delete/`)
      if (!response.ok) {
